...
describe('Post Model', () => {
    describe('Post Creation', () => {
        test("Create a basic post and check the validity of the database entry", async () => {
            ...
            const posted = await makePost(author.id, "This is a new post!", "Woooo new post new post abcdefghhi!!");
    ...
    describe('Post Validation', () => {
        test("Attempt to make posts with invalid or empty author", async() => {
            ...
            await expect(makePost("123123", "asdddd", "Woooo attempting new post abcdefghhi!!")).rejects.toThrowError("SQLITE_CONSTRAINT"); // Non-existant uid
        ...
        test("Attempt to make post that is too short", async() => {
            ...
            await expect(makePost(badAuthor.id, "Short post", "a")).rejects.toThrowError("Validation error: Length Validation Failed");
        });
    });
});
